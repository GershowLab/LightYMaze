...
class MazeDispatcher:
    def __init__(self, ymg: YMazeGeometry, light_controller: LightController = None, choice1rgb=(0, 0, 0),
                 choice2rgb=(0, 0, 255)):
        self._ymg = ymg
        self._maze_mask, self._region_mask = ymg.get_maze_mask()
        self._light_controller = light_controller
        self._maze_minions = [MazeMinion(i, self._maze_mask, self._region_mask, YMazeGeometry.generate_connectivity_matrix(0.1),
                                         self._light_controller, choice1rgb, choice2rgb) for i in
                              range(1, 1 + np.max(self._maze_mask).astype(int))]
    ...
    def open_video(self, fstub):
        ...
        if self._vid_writer is not None:
            ...
            print(
                f"vid writer size = {self._vid_writer.get(cv2.CAP_PROP_FRAME_WIDTH), self._vid_writer.get(cv2.CAP_PROP_FRAME_HEIGHT)}")
        else:
    ...
    def new_frame(self, img: np.ndarray, frame_number: int = None, frame_time: float = None, wait_for_completion=False,
        ...
        if self._img is None:
            ...
            self._img[:] = img
        if multi_thread:
            tt = [mm.new_frame(self._img, frame_number=frame_number, frame_time=frame_time) for mm in
                  self._maze_minions]
...
class MazeMinion:
    def __init__(self, maze_id, maze_mask, region_mask, transition_probs, light_controller=None, choice1rgb=(0, 0, 0),
                 choice2rgb=(0, 0, 255), register_images = True):
        self._pad = 12
        ...
        self._src_h, self._src_w = maze_mask.shape
        self._w = np.clip(w + 2 * self._pad, None, self._src_w - self._x)
        self._h = np.clip(h + 2 * self._pad, None, self._src_h - self._y)
        self._maze_id = maze_id
        self._maze_controller = MazeController(light_controller, self.get_subim(region_mask).copy(), transition_probs,
                                               maze_id, self._pad, choice1rgb, choice2rgb=choice2rgb, register_images=register_images)
    ...
    def enable_image_registration(self, enable):
        self._maze_controller.enable_image_registration(enable)
    ...
    def translate(self, shift):
        # require at least one full pixel (not rounded) shift
        # to avoid flopping back and forth
    ...
    def new_frame_nothread(self, img, frame_number=None, frame_time=None):
        ...
        if self._maze_controller.new_image(roi, frame_number, frame_time):
            self.translate(self._maze_controller.get_shift())
